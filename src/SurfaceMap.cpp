...
namespace TrackQuestFromMap::SurfaceMap
{
	namespace
	{
			...
			const RE::StarMap::SurfaceMapState* a_surfaceState)
		{
			...
			copied.markerCount = (markerEnd - markerBegin) / sizeof(RE::StarMap::SurfaceMarkerStaticData);
			...
			for (std::size_t index = 0; index < copied.markerCount; ++index)
			{
				...
				const auto ownerCount = (ownerEnd - ownerBegin) / sizeof(RE::TESFormID);
	...
	namespace
	{
		...
		void BuildAndSnapshotImpl(RE::StarMap::SurfaceMapState* a_surfaceState)
		{
			...
			{
				logger::error("Rejected invalid Surface Map quest capture");
				return;
			}
